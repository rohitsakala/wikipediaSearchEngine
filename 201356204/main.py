...
class dataContentHandler(xml.sax.ContentHandler):
	...
	def characters(self, data):
		...
		if(self.tag == 'title'):												# Getting title 
			...
			self.docTitle[str(int(self.documentId))] = self.title	# Converting count into Hexadecimal for optimization
			self.pageTag = 0
		elif(self.tag == 'text'):
	...
	def endElement(self, tag):
		...
		if(self.tag == 'page'):
			...
			for documentID in sorted(self.docTitle,key=int):
                		indexFile.write(documentID + "  " + (" ").join(self.docTitle[documentID]) + "\n")
			indexCount += len(self.dic.keys())
		    	print indexCount
		    	self.fileWrite()	
		    	self.dic = {}
	...
	def fileWrite(self):
		...
		for f in sorted(self.dic):
			...
			for g in self.dic[f]:
				...
				word += self.get_val("C",self.dic[f][g][3])
				word += self.get_val("L",self.dic[f][g][4])
...
############### Function's that help in creating data structures for index creation ###########
	...
	def getStopWords(self):
		...
		stopWords.append("ref")
        	stopWords.append("/ref")
        	stopWords.append("references")
        	stopWords.append("reflist")
		for i in stopWords:
			self.stopWords[i] = 1
